...
RegisterCommand(Config.Fines.PayCommand, function(_, args)
    ...
    if not fineId then
        TriggerEvent('vorp:TipRight', 'Uso: /' .. Config.Fines.PayCommand .. ' [id_multa] cash', 5000)
        return
    end

    if args[2] and string.lower(tostring(args[2])) ~= 'cash' then
        TriggerEvent('vorp:TipRight', 'Por ahora solo se aceptan pagos en efectivo.', 5000)
        return
    end

    TriggerServerEvent('daexv_mdt:server:payFine', fineId, method)
end, false)

RegisterNetEvent('daexv_mdt:client:fineReceived')
AddEventHandler('daexv_mdt:client:fineReceived', function(fineData)
    SendNUIMessage({ type = 'fineNotification', data = fineData })
end)
...
AddEventHandler('daexv_mdt:client:myFines', function(fines)
    if not fines or #fines == 0 then
        TriggerEvent('vorp:TipRight', 'No tienes multas pendientes.', 5000)
        return
    end
    ...
    for _, fine in ipairs(fines) do
        TriggerEvent(
            'vorp:TipRight',
            'Multa #' .. fine.id .. ' - $' .. fine.amount .. ' por ' .. fine.charge .. ' (' .. fine.status .. '). Usa /' .. Config.Fines.PayCommand .. ' ' .. fine.id .. ' cash para pagar.',
            8000
        )
        Wait(500)
    end
...
AddEventHandler('daexv_mdt:client:finePaid', function(data)
    data = data or {}
    TriggerEvent('vorp:TipRight', 'Multa #' .. tostring(data.fineId or '') .. ' pagada: $' .. tostring(data.amount or 0), 5000)
end)

RegisterNetEvent('daexv_mdt:client:fineIssued')
AddEventHandler('daexv_mdt:client:fineIssued', function(data)
    SendNUIMessage({ type = 'fineIssued', data = data })
end)
...
CreateThread(function()
    while true do
        ...
        if mdtOpen and IsControlJustReleased(0, 0x156F7119) then
            -- ESC lo maneja la NUI; este hilo mantiene seguro el foco en casos borde de RedM.
        end
    end
end)

AddEventHandler('onResourceStop', function(resourceName)
    if resourceName == GetCurrentResourceName() and mdtOpen then
        mdtOpen = false
        SetNuiFocus(false, false)
    end
end)
...
local function registerCallback(endpoint, serverEvent, buildArgs)
    RegisterNUICallback(endpoint, function(data, cb)
        ...
        handler = AddEventHandler(eventName, function(result)
            SendNUIMessage({ type = 'cb_' .. cbId, data = result })
            if handler then
        ...
        local argsToSend = buildArgs(data, cbId)
...
local function registerAction(endpoint, serverEvent, buildArgs)
    RegisterNUICallback(endpoint, function(data, cb)
        data = data or {}
        local argsToSend = buildArgs(data)
        TriggerServerEvent(serverEvent, unpackArgs(argsToSend))
        cb({ ok = true })
    end)
end
...
registerAction('createIndividual', 'daexv_mdt:server:createIndividual', function(data) return { data.data or data } end)
registerAction('updateIndividual', 'daexv_mdt:server:updateIndividual', function(data) return { data.id, data.data or data } end)
...
registerCallback('getCharges', 'daexv_mdt:server:getCharges', function(data, cbId) return { data.individualId or data.individual_id, cbId } end)
registerAction('addCharge', 'daexv_mdt:server:addCharge', function(data) return { data.data or data } end)
registerAction('updateChargeStatus', 'daexv_mdt:server:updateChargeStatus', function(data) return { data.chargeId or data.charge_id, data.status } end)
registerAction('deleteCharge', 'daexv_mdt:server:deleteCharge', function(data) return { data.chargeId or data.charge_id } end)

registerCallback('getWantedList', 'daexv_mdt:server:getWantedList', function(_, cbId) return { cbId } end)
registerAction('createWarrant', 'daexv_mdt:server:createWarrant', function(data) return { data.data or data } end)
registerAction('signWarrant', 'daexv_mdt:server:signWarrant', function(data) return { data.warrantId or data.warrant_id } end)
registerAction('executeWarrant', 'daexv_mdt:server:executeWarrant', function(data) return { data.warrantId or data.warrant_id } end)
registerAction('cancelWarrant', 'daexv_mdt:server:cancelWarrant', function(data) return { data.warrantId or data.warrant_id, data.reason } end)

registerCallback('getGangs', 'daexv_mdt:server:getGangs', function(_, cbId) return { cbId } end)
registerCallback('getGang', 'daexv_mdt:server:getGang', function(data, cbId) return { data.gangId or data.id, cbId } end)
registerAction('createGang', 'daexv_mdt:server:createGang', function(data) return { data.data or data } end)
registerAction('updateGang', 'daexv_mdt:server:updateGang', function(data) return { data.gangId or data.id, data.data or data } end)
...
registerCallback('getCase', 'daexv_mdt:server:getCase', function(data, cbId) return { data.caseId or data.id, cbId } end)
registerAction('createCase', 'daexv_mdt:server:createCase', function(data) return { data.data or data } end)
registerAction('updateCase', 'daexv_mdt:server:updateCase', function(data) return { data.caseId or data.id, data.data or data } end)
registerAction('updateCaseState', 'daexv_mdt:server:updateCaseState', function(data) return { data.caseId or data.id, data.state } end)
registerAction('archiveCase', 'daexv_mdt:server:archiveCase', function(data) return { data.caseId or data.id } end)
...
registerAction('sendTelegram', 'daexv_mdt:server:sendTelegram', function(data) return { data.data or data } end)
...
registerAction('markTelegramRead', 'daexv_mdt:server:markTelegramRead', function(data) return { data.telegramId or data.telegram_id } end)
registerAction('deleteTelegram', 'daexv_mdt:server:deleteTelegram', function(data) return { data.telegramId or data.telegram_id } end)
registerCallback('getOfficersList', 'daexv_mdt:server:getOfficersList', function(_, cbId) return { cbId } end)

registerAction('issueFine', 'daexv_mdt:server:issueFine', function(data) return { data.data or data } end)
registerCallback('getFines', 'daexv_mdt:server:getFines', function(data, cbId) return { data.individualId or data.individual_id, cbId } end)
registerAction('cancelFine', 'daexv_mdt:server:cancelFine', function(data) return { data.fineId or data.fine_id, data.reason } end)
registerAction('waiveFine', 'daexv_mdt:server:waiveFine', function(data) return { data.fineId or data.fine_id, data.reason } end)
...
registerAction('registerInmate', 'daexv_mdt:server:registerInmate', function(data) return { data.data or data } end)
...
registerCallback('getInmate', 'daexv_mdt:server:getInmate', function(data, cbId) return { data.inmateId or data.id, cbId } end)
...
registerAction('collectEvidence', 'daexv_mdt:server:collectEvidence', function(data) return { data.data or data } end)
registerCallback('getEvidenceForCase', 'daexv_mdt:server:getEvidenceForCase', function(data, cbId) return { data.caseId, cbId } end)
registerAction('attachEvidenceToCase', 'daexv_mdt:server:attachEvidenceToCase', function(data) return { data.caseId, data.data or data } end)
...
registerAction('assignLabor', 'daexv_mdt:server:assignLabor', function(data) return { data.data or data } end)
